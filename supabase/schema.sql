...
create policy "mapping own or admin select"
on public.mashgiach_locations
for select
using (mashgiach_user_id = auth.uid() or public.current_user_role() = 'admin');

create policy "visit logs own or admin select"
...
begin
  ...
  if v_profile.role <> 'mashgiach' then
    raise exception 'Only mashgiach users can register visits';
  end if;
  ...
  select exists (
    select 1
    from public.mashgiach_locations
    where mashgiach_user_id = v_profile.user_id
